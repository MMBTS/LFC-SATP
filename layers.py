...
class general_conv3d_prenorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class ada(nn.Module):
    def __init__(self, in_ch, out_ch, k_size=3, stride=1, padding=1, pad_type='reflect', norm='in', is_training=True, act_type='softmax', relufactor=0.2):
...
class fusion_postnorm(nn.Module):
    def __init__(self, in_channel=64, num_cls=4, id=128, hd=64):
        ...
        self.relu = nn.ReLU()
        self.fc = nn.Linear(id, hd)
        self.out_fc = nn.Linear(hd, 1, bias=False)
    ...
    def forward(self, x, mask):
        ...
        # t1 = y[:, 2:3, ...] 
        # t2 = y[:, 3:4, ...]
        # score1 =  self.relu(self.fc)

        y = y.reshape(B*K, C, H*W*Z)
        rm = y
        w = self.soft(self.out_fc(self.relu(self.fc(rm))))
        ws = (w*y).view(B, K, C, H, W, Z)
